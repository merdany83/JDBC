...
//        while(rs.next()){
//            System.out.println(rs.getObject("Region_id")+" "+rs.getObject("Region_Name"));
            Statement stmt = conn.createStatement();
            ResultSet cnties = stmt.executeQuery("SELECT * FROM countries");
            while(cnties.next()){
                System.out.println(cnties.getObject("country_id")+" "+cnties.getObject("country_name"));
